...
The attack starts with an unauthenticated HTTP request carrying a malformed `Authorization: Basic` header. The parser decodes the value into `v119` and immediately calls `strchr(v119, ':')`, assuming that decoded credentials contain a colon. It then writes a NUL byte through the returned pointer.

For a decoded value without `:`, `strchr` returns NULL. The unchecked write consequently dereferences address zero in `httpd` before authentication completes. The PoC sends the malformed header and the serial log records SIGSEGV with `BadVA=00000000`.
